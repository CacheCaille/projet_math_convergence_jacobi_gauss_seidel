...
# ===========================
# Fonctions Gauss-Seidel et matrices
# ===========================
...
def gauss_seidel(A, b, tol=1e-3, max_iter=200):
    n = len(b)
    x = np.zeros((n, 1))
...
# ===========================
def generate_dataset(nb_samples=1000, taille=3):
    X = []
    Y = []
    it_counts = []
...
model = models.Sequential([
    ...
    layers.Dense(64, activation='relu'),
    layers.Dense(Y_train.shape[1])  # 3 sorties (x1, x2, x3)
])
...
plt.title("Courbe d'apprentissage du MLP (TensorFlow, Gauss-Seidel)")
plt.legend()
plt.show()

# ===========================
# Histogramme du nombre d'itérations Gauss-Seidel
# ===========================
plt.figure()
plt.hist(iterations, bins=20, color='skyblue', edgecolor='black')
plt.xlabel("Nombre d'itérations Gauss-Seidel")
plt.ylabel("Fréquence")
plt.title("Histogramme des itérations Gauss-Seidel")
plt.show()
...
plt.title("Comparaison prédictions vs réel (TensorFlow, Gauss-Seidel)")
plt.legend()
plt.show()
